...
const cityCenters = { 'Казань': [55.796, 49.108], 'Москва': [55.7558, 37.6176], 'Санкт-Петербург': [59.9343, 30.3351] };
const categorySvg = {
...
const EventMap = ({ events, onJoin, onLeave, onDelete, userId, onEventClick, joinedIds = [], likedIds = [], onToggleLike, city = 'Казань' }) => {
  ...
  const icons = useMemo(() => Object.fromEntries(geoEvents.map((event) => [event.id, L.divIcon({
    ...
    html: `<div class="event-map-marker ${markerColor[event.category] || 'blue'}"><span class="marker-symbol">${categorySvg[event.category] || categorySvg['Прогулка']}</span><span><b>${event.category}</b><small>${event.distance}</small></span></div>`,
    iconSize: [150, 54],
    iconAnchor: [26, 51]
  })])), [geoEvents]);
  ...
  return (
    <div className="map-container map-screen">
      <MapContainer key={city} center={cityCenters[city] || cityCenters['Казань']} zoom={12} zoomControl={false} scrollWheelZoom>
        ...
        {geoEvents.map((event) => <Marker key={event.id} position={[event.lat, event.lng]} icon={icons[event.id]} eventHandlers={{ click: () => setActiveEvent(event) }} />)}
      </MapContainer>
